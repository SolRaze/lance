...
(function () {
    ...
    function loadCfg() {
        try { const s = GM_getValue("lance_cfg"); if (s) return Object.assign({}, DEFAULTS, JSON.parse(s)); }
        catch(_) {}
    ...
    async function fileExport(fmt) {
        ...
        if (P === "deepseek") {
            ...
            for (let i = 0; i < items.length - 1; i++)
                if (items[i].role === 'user' && items[i+1]?.role === 'assistant') pl.push({ q: items[i].text, a: items[i+1].text });
    ...
    window.addEventListener("keypress", e => {
        ...
        if (e.key === "Enter" && !e.ctrlKey && !e.shiftKey && !e.metaKey && !e.altKey) {
            const t = getEventTarget(e); if (isEditableTarget(t)) e.stopPropagation();
        }
    ...
    function openDashboard() {
        ...
        const row = (a, b) => {
            ...
            d.appendChild(la); if (b) d.appendChild(b);
            return d;
        };
        const toggle = (val, onChange) => {
            ...
            Object.assign(sl.style, {position:'absolute',inset:'0',borderRadius:'20px',cursor:'pointer',background:val?acc:bd,transition:'background 0.2s'});
            const dot = document.createElement('span');
            Object.assign(dot.style, {position:'absolute',height:'14px',width:'14px',left:val?'19px':'3px',bottom:'3px',background:'#fff',borderRadius:'50%',transition:'left 0.2s'});
            sl.appendChild(dot);
            inp.onchange = () => { const v = inp.checked; sl.style.background = v?acc:bd; dot.style.left = v?'19px':'3px'; onChange(v); };
            lbl.appendChild(inp); lbl.appendChild(sl);
            return lbl;
        };
        const section = (title) => {
            const d = document.createElement('div');
            Object.assign(d.style, {fontSize:'11px',fontWeight:'700',letterSpacing:'0.08em',textTransform:'uppercase',color:isDark?'rgba(255,255,255,0.35)':'rgba(0,0,0,0.4)',padding:'16px 0 6px'});
        ...
        Object.assign(relayStatus.style, {fontSize:'12px',opacity:'0.6'});
        relayRow.appendChild(relayLbl); relayRow.appendChild(relayStatus);
        dlg.appendChild(relayRow);
        GM_xmlhttpRequest({
            method:'GET', url:'http://127.0.0.1:27184/obsidian', timeout:1000,
        ...
        // ── FIX: folderRow was used but never declared — declared here ──
        ...
        Object.assign(folderInp.style, {width:'100%',boxSizing:'border-box',padding:'6px 10px',background:sbg,border:`1px solid ${bd}`,borderRadius:'6px',color:fg,fontSize:'13px'});
        ...
        Object.assign(closeInp.style, {width:'72px',padding:'5px 8px',background:sbg,border:`1px solid ${bd}`,borderRadius:'6px',color:fg,fontSize:'13px',textAlign:'right'});
